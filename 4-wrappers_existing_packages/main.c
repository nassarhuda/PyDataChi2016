...
// added by Huda from here: http://stackoverflow.com/questions/1706551/parse-string-into-argv-argc
...
void process_file(const char* fname,char* cmd_input) {// 

    char **myargv;
    int myargc;
    makeargs(cmd_input, &myargc, &myargv);
    
    int result = parse_cmd_options(myargc, myargv, &opts);
    optind = 1;
